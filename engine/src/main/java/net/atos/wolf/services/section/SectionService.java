...
public class SectionService {
    ...
    public SectionService() {
        ...
        try {
            ...
            sections = ServiceUtilities.OBJECT_MAPPER.readValue(                                 // read the JSON file into a list of type Section
                            SectionService.class.getResourceAsStream(jsonFile),                  // load the file from classpath
                            new TypeReference<List<Section>>() {})                               // tell JACKSON that there is a list in the fiel ([]) and not a single Java  object
                    .stream()
                    .collect(Collectors.toMap(Section::getSectionNumber, Function.identity()));  // convert the list to a map with the section number as key
    ...
    public static void main(String[] args) {
        try {
            InputStream imageStream = SectionService.class.getResourceAsStream("/img/1.png");
            String base64Image = "<img src='data:image/png;base64," + Base64.getEncoder().encodeToString(imageStream.readAllBytes()) + "'>";
